...
export const BlogCard = ({ id, authorName, title, content, publishedDate, thumbnail }: BlogCardProps) => {
  const words = content.split(" ");
  const excerpt = words.length > 20 ? words.slice(0, 20).join(" ") : content;
    ...
    <Link href={`/blog/${id}`}>
      <div className="p-4 transition-transform duration-300 hover:scale-[1.02]">
        <div className="flex flex-col bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden md:flex-row md:max-w-2xl hover:shadow-md transition-shadow duration-300">
          <div className="relative w-full h-64 md:h-48 md:w-48 shrink-0">
            <Image
              src={thumbnail}
              alt={title}
              fill
